...
class Vehicle:
    def __init__(
        ...
        exit_on_stale_can=False,
    ):
        self.max_vel = np.array(max_vel)
        self.max_accel = np.array(max_accel)
        self.exit_on_stale_can = exit_on_stale_can
    ...
    def update_state(self):
        ...
        status = phoenix6.BaseStatusSignal.refresh_all(self.status_signals)  # Note: Signal latency is roughly 4 ms
        if self.exit_on_stale_can and status == StatusCode.CAN_MESSAGE_STALE:
            print('CAN message is stale; killing gamepad teleop')

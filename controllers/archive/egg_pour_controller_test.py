...
DEFAULT_GEMINI_BOWL_RESPONSE_PATH = (
    _CONTROLLERS.parent / "logs" / "gemini_response_egg_pour_test_bowl.png"
)
DEFAULT_GEMINI_PAN_RESPONSE_PATH = (
    _CONTROLLERS.parent / "logs" / "gemini_response_egg_pour_test_pan.png"
)

BOWL_CARRY_LIFT_M = 0.10
BOWL_PLACE_APPROACH_DZ_M = 0.10
...
DETECTION_EE_POSITION = np.array([0.1930, -0.1954, 0.6736], dtype=np.float64)
...
def _detect_pan_center(
    ...
    if radius_m is not None:
        print(
            f"[egg_pour_test] detected pan center={center.tolist()} radius={radius_m:.3f} m"
        )
    else:
        print(f"[egg_pour_test] detected pan center={center.tolist()} (radius unavailable)")
    return center
...
def run_egg_pour_test_cycle(
    ...
    *,
    ...
    arm.move_to(
        ...
        ARM_HOME_POSITION,
        ARM_HOME_ORIENTATION,
        label=f"[egg_pour_test] move to home {ARM_HOME_POSITION.tolist()}",
    ...
    arm.move_to(
        ...
        detection_pos,
        ARM_HOME_ORIENTATION,
        label=f"[egg_pour_test] move to shared detection pose {detection_pos.tolist()}",
        tol_m=HOME_POS_TOL_M,
    )
    ...
    print(f"[egg_pour_test] black bowl grasp pose: {pick_pos.tolist()}")
    grasp.object(
        ctx,
        Object.PASTA_BOWL,
        pick_pos=pick_pos,
    ...
    arm.move_to(
        ...
        pour_start_pos,
        grip_R,
        label=f"[egg_pour_test] move above pan center {pour_start_pos.tolist()}",
    )
    ...
    try:
        arm.move_to(
            ...
            tilted_ori,
            label=(
                f"[egg_pour_test] tilt {pour_tilt_deg:.0f}° about world "
        ...
        arm.move_to(
            ...
            pour_start_pos,
            straight_down_R,
            label=f"[egg_pour_test] return upright to {pour_start_pos.tolist()}",
            tol_m=POUR_TILT_TOL_M,
            timeout_s=8.0,
        )
    finally:
        gains.restore_precise_grasp(ctx.redis, precise, label="egg_pour_test tilt")

    grasp.place(ctx, Object.PASTA_BOWL, place_pos=pick_pos, ori=grip_R)
    print("[egg_pour_test] bowl replaced at original pick pose.")
    ...
    if return_home:
        arm.move_to(
            ...
            ARM_HOME_POSITION,
            ARM_HOME_ORIENTATION,
            label=f"[egg_pour_test] return to home {ARM_HOME_POSITION.tolist()}",
            tol_m=HOME_POS_TOL_M,
        )
